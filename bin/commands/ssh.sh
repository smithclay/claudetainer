...
# Detect the SSH username based on devcontainer configuration
get_devcontainer_user() {
    local devcontainer_file=".devcontainer/claudetainer/devcontainer.json"

    if [[ -f $devcontainer_file ]]; then
        # Extract USERNAME from sshd feature configuration
        local sshd_username
        sshd_username=$(grep -A 10 "ghcr.io/devcontainers/features/sshd" "$devcontainer_file" | grep '"USERNAME"' | sed 's/.*"USERNAME":[[:space:]]*"\([^"]*\)".*/\1/')
        if [[ -n $sshd_username ]]; then
            echo "$sshd_username"
            return 0
        fi
    fi

    # Fallback to vscode if we can't determine from config
    echo "vscode"
}
...
cmd_ssh() {
    ...
    # Get the correct username for this devcontainer
    local ssh_user
    ssh_user=$(get_devcontainer_user)

    ui_print_info "Connecting to container via SSH on port $port as user: $ssh_user"
    ...
    if [[ -n $ssh_key_args ]]; then
        ...
        if "${ssh_with_key_cmd[@]}" -o BatchMode=yes -o ConnectTimeout=5 "$ssh_user@localhost" exit 2>/dev/null; then
            # Key auth works, connect normally
            "${ssh_with_key_cmd[@]}" "$ssh_user@localhost"
            return 0
        else
            ...
            if "${ssh_base_cmd[@]}" -o ConnectTimeout=10 "$ssh_user@localhost" "sudo /workspaces/.devcontainer/claudetainer/setup-ssh-keys.sh" 2>/dev/null; then
    ...
    "${ssh_base_cmd[@]}" "$ssh_user@localhost"
}
