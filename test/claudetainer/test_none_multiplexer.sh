...
check "auto-start script exists" test -f ~/.claude/scripts/bashrc-multiplexer.sh
...
check "auto-start script contains welcome message" grep -q "Welcome to Claudetainer" ~/.claude/scripts/bashrc-multiplexer.sh

# Test 8: Check that auto-start script mentions no multiplexer
check "auto-start script mentions no multiplexer" grep -q "No multiplexer configured" ~/.claude/scripts/bashrc-multiplexer.sh

# Test 9: Check that auto-start script changes to workspaces directory
check "auto-start script changes to workspaces" grep -q "/workspaces" ~/.claude/scripts/bashrc-multiplexer.sh
